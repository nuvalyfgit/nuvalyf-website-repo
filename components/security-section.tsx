      ...
      <div className="site-container">
        ...
        <div className="reveal-stagger mt-14 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {securityFeatures.map((item, index) => {
            ...
            return <article key={item.title} className="card-lift rounded-[22px] border border-[#e1ebe9] bg-white p-6"><span className="grid h-11 w-11 place-items-center rounded-2xl bg-[#0b1535] text-[#39dbc0]"><Icon className="h-5 w-5"/></span><h3 className="mt-5 text-base font-bold text-[#0b1535]">{item.title}</h3><p className="mt-2 text-sm leading-6 text-[#647383]">{item.description}</p></article>;
          })}
        </div>
      </div>
    </section>
