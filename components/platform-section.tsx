...
function ProductVisual({ type }: { type: string }) {
  ...
  return <div className="product-panel p-4"><div className="grid grid-cols-3 gap-2">{[["Visits","684"],["Appointments","522"],["Collections","₹3.8L"]].map(([label,value])=><div key={label} className="rounded-xl border border-[#e1eae8] bg-white p-3"><span className="text-[9px] text-[#7e8a95]">{label}</span><strong className="mt-1 block text-sm text-[#0b1535]">{value}</strong></div>)}</div><div className="mt-3 rounded-xl bg-[#0b1535] p-4"><div className="flex items-center justify-between"><span className="text-[10px] font-bold text-white">Clinic activity</span><span className="text-[9px] text-white/60">Last 7 days</span></div><div className="mt-6 flex h-28 items-end gap-3">{[42,66,51,82,70,90,76].map((height,index)=><span key={index} className="flex-1 rounded-t-md bg-gradient-to-t from-[#39dbc0] to-[#7d86ff]" style={{height:`${height}%`}}/>)}</div></div><div className="mt-3 flex items-center gap-2 rounded-xl bg-[#e8f7f4] p-3 text-[10px] font-semibold text-[#087a70]"><FileCheck2 className="h-4 w-4"/> Clinic data export ready when authorised</div></div>;
}

export function PlatformSection() {
  return (
    <section id="platform" className="grain relative overflow-hidden bg-[#070d24] py-24 sm:py-32">
